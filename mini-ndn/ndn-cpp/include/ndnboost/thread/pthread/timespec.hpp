#ifndef NDNBOOST_THREAD_PTHREAD_TIMESPEC_HPP
#define NDNBOOST_THREAD_PTHREAD_TIMESPEC_HPP
...
#     define NDNBOOST_THREAD_TIMESPEC_MAC_API
...
namespace ndnboost
{
  ...
  {
#if defined NDNBOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(ndnboost::system_time const& abs_time)
    {
...
#endif
#if defined NDNBOOST_THREAD_USES_CHRONO
    ...
    inline timespec to_timespec(ndnboost::intmax_t const& ns)
    {
      ndnboost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline ndnboost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
      return static_cast<ndnboost::intmax_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
    ...
    inline timespec timespec_now()
    {
...
#if defined(NDNBOOST_THREAD_TIMESPEC_MAC_API)
      timeval tv;
      ::gettimeofday(&tv, 0);
      ts.tv_sec = tv.tv_sec;
      ...
      {
        NDNBOOST_ASSERT(0 && "Boost::Thread - Internal Error");
      }
#endif
      return ts;
    }
