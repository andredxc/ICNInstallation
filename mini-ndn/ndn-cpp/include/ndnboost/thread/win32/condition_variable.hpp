#ifndef NDNBOOST_THREAD_CONDITION_VARIABLE_WIN32_HPP
#define NDNBOOST_THREAD_CONDITION_VARIABLE_WIN32_HPP
...
namespace ndnboost
{
    namespace detail
    {
        ...
        public:
            NDNBOOST_THREAD_NO_COPYABLE(basic_cv_list_entry)
            ...
            static bool no_waiters(ndnboost::intrusive_ptr<basic_cv_list_entry> const& entry)
            {
            ...
            void add_waiter()
            {
                NDNBOOST_INTERLOCKED_INCREMENT(&waiters);
            }

            void remove_waiter()
            {
                NDNBOOST_INTERLOCKED_DECREMENT(&waiters);
            }
            ...
            bool woken()
            {
                ...
                NDNBOOST_ASSERT((woken_result==detail::win32::timeout) || (woken_result==0));
        ...
        {
            NDNBOOST_INTERLOCKED_INCREMENT(&p->references);
        }

        inline void intrusive_ptr_release(basic_cv_list_entry * p)
        {
            if(!NDNBOOST_INTERLOCKED_DECREMENT(&p->references))
            {
                delete p;
            }
        }
        ...
        {
            ndnboost::mutex internal_mutex;
            long total_count;
            unsigned active_generation_count;

            typedef basic_cv_list_entry list_entry;

            typedef ndnboost::intrusive_ptr<list_entry> entry_ptr;
            ...
            struct relocker
            {
                NDNBOOST_THREAD_NO_COPYABLE(relocker)
                lock_type& lock;
                bool unlocked;
            ...
            {
                ndnboost::lock_guard<ndnboost::mutex> internal_lock(internal_mutex);

                if(!wake_sem)
                {
                    wake_sem=detail::win32::create_anonymous_semaphore(0,LONG_MAX);
                    NDNBOOST_ASSERT(wake_sem);
                }
            ...
            struct entry_manager
            {
                ...
                NDNBOOST_THREAD_NO_COPYABLE(entry_manager)
                entry_manager(entry_ptr const& entry_, ndnboost::mutex& mutex_):
    ...
    public:
...
#if defined NDNBOOST_THREAD_USES_DATETIME
        bool timed_wait(unique_lock<mutex>& m,ndnboost::system_time const& abs_time)
        {
            return do_wait(m,abs_time);
        }

        bool timed_wait(unique_lock<mutex>& m,ndnboost::xtime const& abs_time)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,ndnboost::system_time const& abs_time,predicate_type pred)
        {
            return do_wait(m,abs_time,pred);
        }
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,ndnboost::xtime const& abs_time,predicate_type pred)
        {
    ...
    public:
        ...
        template<typename lock_type>
        bool timed_wait(lock_type& m,ndnboost::xtime const& abs_time)
        {
            return do_wait(m,system_time(abs_time));
        }
        ...
        template<typename lock_type,typename predicate_type>
        bool timed_wait(lock_type& m,ndnboost::xtime const& abs_time,predicate_type pred)
        {
            return do_wait(m,system_time(abs_time),pred);
        }
