#ifndef NDNBOOST_THREAD_WIN32_SHARED_MUTEX_HPP
#define NDNBOOST_THREAD_WIN32_SHARED_MUTEX_HPP
...
namespace ndnboost
{
    ...
    public:
        NDNBOOST_THREAD_NO_COPYABLE(shared_mutex)
        shared_mutex()
        {
        ...
        void lock_shared()
        {
            ...
            NDNBOOST_VERIFY(timed_lock_shared(::ndnboost::detail::get_system_time_sentinel()));
#else
            NDNBOOST_VERIFY(try_lock_shared_until(chrono::steady_clock::now()));
#endif
        }
        ...
        bool timed_lock_shared(ndnboost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                for(;;)
                {
                    ...
                    if(new_state.exclusive || new_state.exclusive_waiting_blocked)
                    {
                        ...
                        {
                            ndnboost::throw_exception(ndnboost::lock_error());
                        }
                    }
                    else
                    {
                ...
                unsigned long const res=detail::win32::WaitForSingleObject(semaphores[unlock_sem],::ndnboost::detail::get_milliseconds_until(wait_until));
            ...
            for(;;)
            {
                ...
                if(current_state==old_state)
                {
                    if(last_reader)
                    {
                        ...
                        {
                            NDNBOOST_VERIFY(detail::win32::ReleaseSemaphore(upgrade_sem,1,0)!=0);
                        }
                        else
                        {
        ...
        {
#if defined NDNBOOST_THREAD_USES_DATETIME
            NDNBOOST_VERIFY(timed_lock(::ndnboost::detail::get_system_time_sentinel()));
#else
            NDNBOOST_VERIFY(try_lock_until(chrono::steady_clock::now()));
#endif
        }
        ...
        bool timed_lock(ndnboost::system_time const& wait_until)
        {
            for(;;)
            {
                ...
                #endif
                unsigned long const wait_res=detail::win32::WaitForMultipleObjects(2,semaphores,wait_all,::ndnboost::detail::get_milliseconds_until(wait_until));
                if(wait_res==detail::win32::timeout)
                {
                    for(;;)
                    {
                        ...
                        if (must_notify)
                        {
                          NDNBOOST_VERIFY(detail::win32::ReleaseSemaphore(semaphores[unlock_sem],1,0)!=0);
                        }
        ...
        void lock_upgrade()
        {
            for(;;)
            {
                ...
                NDNBOOST_VERIFY(!detail::win32::WaitForSingleObject(semaphores[unlock_sem],detail::win32::infinite));
            }
        }
        ...
        void unlock_upgrade_and_lock()
        {
            ...
            for(;;)
            {
                ...
                if(current_state==old_state)
                {
                    if(!last_reader)
                    {
                        NDNBOOST_VERIFY(!detail::win32::WaitForSingleObject(upgrade_sem,detail::win32::infinite));
                    }
                    break;
                }
                old_state=current_state;
