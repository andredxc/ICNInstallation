#ifndef NDNBOOST_WIN32_THREAD_PRIMITIVES_HPP
#define NDNBOOST_WIN32_THREAD_PRIMITIVES_HPP
...
//#include <ndnboost/detail/winapi/synchronization.hpp>
#include <algorithm>
...
#if !defined(NDNBOOST_NO_ANSI_APIS)
...
#pragma intrinsic(_InterlockedCompareExchange)
            ...
            {
                NDNBOOST_VERIFY(ReleaseSemaphore(semaphore,count,0)!=0);
            }

            class NDNBOOST_THREAD_DECL handle_manager
            {
            private:
                ...
                void cleanup()
                {
                    ...
                    {
                        NDNBOOST_VERIFY(CloseHandle(handle_to_manage));
                    }
                }
...
#if defined(NDNBOOST_MSVC) && (_MSC_VER>=1400)  && !defined(UNDER_CE)
...
#define NDNBOOST_THREAD_BTS_DEFINED
#elif (defined(NDNBOOST_MSVC) || defined(NDNBOOST_INTEL_WIN)) && defined(_M_IX86)
namespace ndnboost
{
    namespace detail
    {
...
#ifndef NDNBOOST_THREAD_BTS_DEFINED
    ...
    namespace detail
    {
        namespace win32
        {
            inline bool interlocked_bit_test_and_set(long* x,long bit)
            {
                ...
                do
                {
                    long const current=NDNBOOST_INTERLOCKED_COMPARE_EXCHANGE(x,old|value,old);
                    if(current==old)
                    {
                        break;
                    }
