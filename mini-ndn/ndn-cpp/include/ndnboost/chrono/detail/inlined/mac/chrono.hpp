...
system_clock::now() NDNBOOST_NOEXCEPT
{
    timeval tv;
    gettimeofday(&tv, 0);
...
time_t
system_clock::to_time_t(const time_point& t) NDNBOOST_NOEXCEPT
{
...
system_clock::time_point
system_clock::from_time_t(time_t t) NDNBOOST_NOEXCEPT
{
...
steady_full()
{
    ...
    if (err != 0) 
    {
      NDNBOOST_ASSERT(0 && "Boost::Chrono - Internal Error");
    }
...
steady_full_ec(system::error_code & ec)
{
    ...
    if (err != 0) 
    {
        ...
        {
            ec.assign( errno, NDNBOOST_CHRONO_SYSTEM_CATEGORY );
...
steady_clock::now() NDNBOOST_NOEXCEPT
{
    static kern_return_t err;
...
steady_clock::now(system::error_code & ec)
{
    ...
    if ( err != 0  ) 
    {
        ...
        {
            ec.assign( err, NDNBOOST_CHRONO_SYSTEM_CATEGORY );
            return time_point();
        }
    }
    if (!NDNBOOST_CHRONO_IS_THROWS(ec)) 
