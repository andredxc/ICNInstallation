...
//      workaround for NDNBOOST_NO_STDC_NAMESPACE (the library
...
#ifndef NDNBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
#define NDNBOOST_TOKEN_FUNCTIONS_JRB120303_HPP_
...
#if !defined(NDNBOOST_NO_CWCTYPE)
#include <cwctype>
#endif
...
// MSVC does not like the following typename
  template <class Char,
    class Traits = NDNBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  template <typename Char,
    typename Tr = NDNBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
  ...
  // The out of the box GCC 2.95 on cygwin does not have a char_traits class.
  template <class Char,
    class Tr = NDNBOOST_DEDUCED_TYPENAME std::basic_string<Char>::traits_type >
