#ifndef NDNBOOST_CORE_REF_HPP
#define NDNBOOST_CORE_REF_HPP
...
#if defined( NDNBOOST_MSVC ) && NDNBOOST_WORKAROUND( NDNBOOST_MSVC, == 1600 )
...
public:
    ...
    */
    NDNBOOST_FORCEINLINE explicit reference_wrapper(T& t): t_(ndnboost::addressof(t)) {}

#if defined( NDNBOOST_MSVC ) && NDNBOOST_WORKAROUND( NDNBOOST_MSVC, == 1600 )

    NDNBOOST_FORCEINLINE explicit reference_wrapper( T & t, ref_workaround_tag ): t_( ndnboost::addressof( t ) ) {}
    ...
    */
    NDNBOOST_DELETED_FUNCTION(reference_wrapper(T&& t))
public:
#endif
    ...
    /**
     ...
     @remark Does not throw.
    */
    NDNBOOST_FORCEINLINE operator T& () const { return *t_; }

    /**
     @return The stored reference.
     @remark Does not throw.
    */
    NDNBOOST_FORCEINLINE T& get() const { return *t_; }
    ...
    /**
     ...
     @remark Does not throw.
    */
    NDNBOOST_FORCEINLINE T* get_pointer() const { return t_; }
...
/**
 ...
 @remark Does not throw.
*/
template<class T> NDNBOOST_FORCEINLINE reference_wrapper<T> NDNBOOST_REF_CONST ref( T & t )
{
#if defined( NDNBOOST_MSVC ) && NDNBOOST_WORKAROUND( NDNBOOST_MSVC, == 1600 )
...
/**
 ...
 @remark Does not throw.
*/
template<class T> NDNBOOST_FORCEINLINE reference_wrapper<T const> NDNBOOST_REF_CONST cref( T const & t )
{
    return reference_wrapper<T const>(t);
}
...
#  define NDNBOOST_REF_DELETE
#else
#  define NDNBOOST_REF_DELETE = delete
#endif
/**
 @endcond
*/
...
/**
 @remark Construction from a temporary object is disabled.
*/
template<class T> void ref(T const&&) NDNBOOST_REF_DELETE;

/**
 @remark Construction from a temporary object is disabled.
*/
template<class T> void cref(T const&&) NDNBOOST_REF_DELETE;

#undef NDNBOOST_REF_DELETE
...
#endif // !defined(NDNBOOST_NO_CV_SPECIALIZATIONS)
...
/**
 ...
 @remark Does not throw.
*/
template<class T> NDNBOOST_FORCEINLINE typename unwrap_reference<T>::type& unwrap_ref( T & t )
{
    return t;
}
...
/**
 @cond
*/
template<class T> NDNBOOST_FORCEINLINE T* get_pointer( reference_wrapper<T> const & r )
{
...
#endif // #ifndef NDNBOOST_CORE_REF_HPP
