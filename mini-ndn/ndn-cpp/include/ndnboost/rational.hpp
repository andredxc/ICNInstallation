...
//  22 Jun 00  Change _MSC_VER to NDNBOOST_MSVC so other compilers are not
...
#ifndef NDNBOOST_RATIONAL_HPP
#define NDNBOOST_RATIONAL_HPP

#include <iostream>              // for std::istream and std::ostream
#include <ios>                   // for std::noskipws
#include <stdexcept>             // for std::domain_error
#include <string>                // for std::string implicit constructor
#include <ndnboost/operators.hpp>   // for ndnboost::addable etc
#include <cstdlib>               // for std::abs
#include <ndnboost/call_traits.hpp> // for ndnboost::call_traits
#include <ndnboost/config.hpp>      // for NDNBOOST_NO_STDC_NAMESPACE, NDNBOOST_MSVC
#include <ndnboost/detail/workaround.hpp> // for NDNBOOST_WORKAROUND
#include <ndnboost/assert.hpp>      // for NDNBOOST_ASSERT
#include <ndnboost/math/common_factor_rt.hpp>  // for ndnboost::math::gcd, lcm
#include <limits>                // for std::numeric_limits
#include <ndnboost/static_assert.hpp>  // for NDNBOOST_STATIC_ASSERT

// Control whether depreciated GCD and LCM functions are included (default: yes)
#ifndef NDNBOOST_CONTROL_RATIONAL_HAS_GCD
#define NDNBOOST_CONTROL_RATIONAL_HAS_GCD  1
#endif

namespace ndnboost {

#if NDNBOOST_CONTROL_RATIONAL_HAS_GCD
...
#endif  // NDNBOOST_CONTROL_RATIONAL_HAS_GCD
    ...
    > > > > > > > > > > > > > > > >
{
    ...
    NDNBOOST_STATIC_ASSERT( ::std::numeric_limits<IntType>::is_specialized );

    // Helper types
    typedef typename ndnboost::call_traits<IntType>::param_type param_type;
...
inline T rational_cast(
    const rational<IntType>& src NDNBOOST_APPEND_EXPLICIT_TEMPLATE_TYPE(T))
{
