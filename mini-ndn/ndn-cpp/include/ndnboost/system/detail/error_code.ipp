...
#include <ndnboost/config/warning_disable.hpp>
...
# if defined( NDNBOOST_WINDOWS_API )
#   include <windows.h>
#   if !NDNBOOST_PLAT_WINDOWS_RUNTIME
#     include <ndnboost/system/detail/local_free_on_destruction.hpp>
#   endif
...
namespace
{
  ...
  public:
    ...
    const char *   name() const NDNBOOST_SYSTEM_NOEXCEPT;
  ...
  public:
    ...
    const char *        name() const NDNBOOST_SYSTEM_NOEXCEPT;
    std::string         message( int ev ) const;
    error_condition     default_error_condition( int ev ) const NDNBOOST_SYSTEM_NOEXCEPT;
  };

  //  generic_error_category implementation  ---------------------------------//

  const char * generic_error_category::name() const NDNBOOST_SYSTEM_NOEXCEPT
  {
    return "generic";
  }
  ...
  {
    using namespace ndnboost::system::errc;
#if defined(__PGI)
      using ndnboost::system::errc::invalid_argument;
#endif
  ...
  //      thread safe.
  # if defined(NDNBOOST_WINDOWS_API) || defined(__hpux) || defined(__sun)\
     || (defined(__linux) && (!defined(__USE_XOPEN2K) || defined(NDNBOOST_SYSTEM_USE_STRERROR)))\
...
#   endif
  ...
  const char * system_error_category::name() const NDNBOOST_SYSTEM_NOEXCEPT
  {
    return "system";
  }

  error_condition system_error_category::default_error_condition( int ev ) const NDNBOOST_SYSTEM_NOEXCEPT
  {
    using namespace ndnboost::system::errc;
#if defined(__PGI)
      using ndnboost::system::errc::invalid_argument;
...
# else
  ...
  std::string system_error_category::message( int ev ) const
  {
#if defined(UNDER_CE) || NDNBOOST_PLAT_WINDOWS_RUNTIME || defined(NDNBOOST_NO_ANSI_APIS)
...
# ifndef NDNBOOST_SYSTEM_NO_DEPRECATED
    NDNBOOST_SYSTEM_DECL error_code throws; // "throw on error" special error_code;
...
#   define NDNBOOST_SYSTEM_LINKAGE inline
# else
#   define NDNBOOST_SYSTEM_LINKAGE NDNBOOST_SYSTEM_DECL
# endif

    NDNBOOST_SYSTEM_LINKAGE const error_category & system_category() NDNBOOST_SYSTEM_NOEXCEPT
    {
      static const system_error_category  system_category_const;
      return system_category_const;
    }

    NDNBOOST_SYSTEM_LINKAGE const error_category & generic_category() NDNBOOST_SYSTEM_NOEXCEPT
    {
