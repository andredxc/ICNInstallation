...
#ifndef NDNBOOST_ATOMIC_DETAIL_ATOMIC_TEMPLATE_HPP_INCLUDED_
#define NDNBOOST_ATOMIC_DETAIL_ATOMIC_TEMPLATE_HPP_INCLUDED_
...
#include <ndnboost/atomic/detail/casts.hpp>
...
#pragma warning(push)
// 'ndnboost::atomics::atomic<T>' : multiple assignment operators specified
...
NDNBOOST_FORCEINLINE NDNBOOST_CONSTEXPR memory_order deduce_failure_order(memory_order order) NDNBOOST_NOEXCEPT
{
    return order == memory_order_acq_rel ? memory_order_acquire : (order == memory_order_release ? memory_order_relaxed : order);
}

NDNBOOST_FORCEINLINE NDNBOOST_CONSTEXPR bool cas_failure_order_must_not_be_stronger_than_success_order(memory_order success_order, memory_order failure_order) NDNBOOST_NOEXCEPT
{
...
template< typename T, bool IsInt = ndnboost::is_integral< T >::value >
struct classify
{
    typedef void type;
};
...
private:
    ...
    typedef atomics::detail::operations< storage_size_of< value_type >::value, ndnboost::is_signed< T >::value > operations;
...
public:
    NDNBOOST_DEFAULTED_FUNCTION(base_atomic(), {})
    NDNBOOST_CONSTEXPR explicit base_atomic(value_type v) NDNBOOST_NOEXCEPT : m_storage(v) {}

    NDNBOOST_FORCEINLINE void store(value_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        NDNBOOST_ASSERT(order != memory_order_consume);
    ...
    NDNBOOST_FORCEINLINE value_type load(memory_order order = memory_order_seq_cst) const volatile NDNBOOST_NOEXCEPT
    {
        NDNBOOST_ASSERT(order != memory_order_release);
        NDNBOOST_ASSERT(order != memory_order_acq_rel);

        return static_cast< value_type >(operations::load(m_storage, order));
    }

    NDNBOOST_FORCEINLINE value_type fetch_add(difference_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::fetch_add(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE value_type fetch_sub(difference_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::fetch_sub(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE value_type exchange(value_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::exchange(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE bool compare_exchange_strong(value_type& expected, value_type desired, memory_order success_order, memory_order failure_order) volatile NDNBOOST_NOEXCEPT
    {
        NDNBOOST_ASSERT(failure_order != memory_order_release);
        NDNBOOST_ASSERT(failure_order != memory_order_acq_rel);
        NDNBOOST_ASSERT(cas_failure_order_must_not_be_stronger_than_success_order(success_order, failure_order));
    ...
    NDNBOOST_FORCEINLINE bool compare_exchange_strong(value_type& expected, value_type desired, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return compare_exchange_strong(expected, desired, order, atomics::detail::deduce_failure_order(order));
    }

    NDNBOOST_FORCEINLINE bool compare_exchange_weak(value_type& expected, value_type desired, memory_order success_order, memory_order failure_order) volatile NDNBOOST_NOEXCEPT
    {
        NDNBOOST_ASSERT(failure_order != memory_order_release);
        NDNBOOST_ASSERT(failure_order != memory_order_acq_rel);
        NDNBOOST_ASSERT(cas_failure_order_must_not_be_stronger_than_success_order(success_order, failure_order));
    ...
    NDNBOOST_FORCEINLINE bool compare_exchange_weak(value_type& expected, value_type desired, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return compare_exchange_weak(expected, desired, order, atomics::detail::deduce_failure_order(order));
    }

    NDNBOOST_FORCEINLINE value_type fetch_and(value_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::fetch_and(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE value_type fetch_or(value_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::fetch_or(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE value_type fetch_xor(value_type v, memory_order order = memory_order_seq_cst) volatile NDNBOOST_NOEXCEPT
    {
        return static_cast< value_type >(operations::fetch_xor(m_storage, static_cast< storage_type >(v), order));
    }

    NDNBOOST_FORCEINLINE bool is_lock_free() const volatile NDNBOOST_NOEXCEPT
    {
        return operations::is_lock_free(m_storage);
    }

    NDNBOOST_FORCEINLINE value_type operator++(int) volatile NDNBOOST_NOEXCEPT
    {
        return fetch_add(1);
    }

    NDNBOOST_FORCEINLINE value_type operator++() volatile NDNBOOST_NOEXCEPT
    {
    ...
    NDNBOOST_FORCEINLINE value_type operator+=(difference_type v) volatile NDNBOOST_NOEXCEPT
    {
    ...
    NDNBOOST_FORCEINLINE value_type operator&=(value_type v) volatile NDNBOOST_NOEXCEPT
    {
    ...
    NDNBOOST_DELETED_FUNCTION(base_atomic(base_atomic const&))
    NDNBOOST_DELETED_FUNCTION(base_atomic& operator=(base_atomic const&))
};
...
public:
    NDNBOOST_FORCEINLINE explicit base_atomic(value_type const& v = value_type()) NDNBOOST_NOEXCEPT : m_storage(atomics::detail::memcpy_cast< storage_type >(v))
    {
    }
...
public:
    NDNBOOST_DEFAULTED_FUNCTION(base_atomic(), {})
    NDNBOOST_FORCEINLINE explicit base_atomic(value_type const& v) NDNBOOST_NOEXCEPT : m_storage(atomics::detail::union_cast< storage_type >(v))
    {
    }
...
public:
    NDNBOOST_DEFAULTED_FUNCTION(atomic(), NDNBOOST_NOEXCEPT {})
    ...
    NDNBOOST_FORCEINLINE explicit NDNBOOST_CONSTEXPR atomic(value_arg_type v) NDNBOOST_NOEXCEPT : base_type(v) {}

    NDNBOOST_FORCEINLINE value_type operator= (value_arg_type v) volatile NDNBOOST_NOEXCEPT
    {
        this->store(v);
        return v;
    }

    NDNBOOST_FORCEINLINE operator value_type() volatile const NDNBOOST_NOEXCEPT
    {
        return this->load();
    }

    NDNBOOST_FORCEINLINE storage_type& storage() NDNBOOST_NOEXCEPT { return this->m_storage; }
    NDNBOOST_FORCEINLINE storage_type volatile& storage() volatile NDNBOOST_NOEXCEPT { return this->m_storage; }
    NDNBOOST_FORCEINLINE storage_type const& storage() const NDNBOOST_NOEXCEPT { return this->m_storage; }
    NDNBOOST_FORCEINLINE storage_type const volatile& storage() const volatile NDNBOOST_NOEXCEPT { return this->m_storage; }

    NDNBOOST_DELETED_FUNCTION(atomic(atomic const&))
    NDNBOOST_DELETED_FUNCTION(atomic& operator= (atomic const&))
    NDNBOOST_DELETED_FUNCTION(atomic& operator= (atomic const&) volatile)
};
...
typedef atomic< ndnboost::ulong_long_type > atomic_ullong;
typedef atomic< ndnboost::long_long_type > atomic_llong;
#endif
typedef atomic< void* > atomic_address;
typedef atomic< bool > atomic_bool;
typedef atomic< wchar_t > atomic_wchar_t;
#if !defined(NDNBOOST_NO_CXX11_CHAR16_T)
typedef atomic< char16_t > atomic_char16_t;
#endif
#if !defined(NDNBOOST_NO_CXX11_CHAR32_T)
...
#endif // NDNBOOST_ATOMIC_DETAIL_ATOMIC_TEMPLATE_HPP_INCLUDED_
