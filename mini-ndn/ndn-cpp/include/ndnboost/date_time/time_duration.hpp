...
namespace date_time {
  ...
  */
  ...
  class time_duration : private
      ndnboost::less_than_comparable<T
    , ndnboost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, ndnboost::int64_t frac_of_second>
  ...
  private:
    ...
    NDNBOOST_STATIC_ASSERT_MSG((traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second % frac_of_second : frac_of_second % traits_type::ticks_per_second) == 0,\
      "The base duration resolution must be a multiple of the subsecond duration resolution");
    NDNBOOST_STATIC_CONSTANT(ndnboost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    explicit subsecond_duration(ndnboost::int64_t ss) :
