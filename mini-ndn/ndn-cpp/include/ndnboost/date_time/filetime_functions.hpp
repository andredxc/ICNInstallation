 ...
 * NDNBOOST_HAS_FTIME defined.
 */

#include <ndnboost/date_time/compiler_config.hpp>

#if defined(NDNBOOST_HAS_FTIME) // skip this file if no FILETIME
...
#include <ndnboost/date_time/time.hpp>
#include <ndnboost/date_time/date_defs.hpp>
...
namespace winapi {
    ...
    extern "C" {
        ...
        struct FILETIME
        {
            ndnboost::uint32_t dwLowDateTime;
            ndnboost::uint32_t dwHighDateTime;
        };
        struct SYSTEMTIME
        {
            ndnboost::uint16_t wYear;
            ndnboost::uint16_t wMonth;
            ndnboost::uint16_t wDayOfWeek;
            ndnboost::uint16_t wDay;
            ndnboost::uint16_t wHour;
            ndnboost::uint16_t wMinute;
            ndnboost::uint16_t wSecond;
            ndnboost::uint16_t wMilliseconds;
        };
...
#elif defined(NDNBOOST_HAS_GETSYSTEMTIMEASFILETIME)
     ...
     *       ndnboost::date_time::winapi::file_time type.
     */
    template< typename FileTimeT >
    inline ndnboost::uint64_t file_time_to_microseconds(FileTimeT const& ft)
    {
...
#if !defined(NDNBOOST_DATE_TIME_POSIX_TIME_STD_CONFIG)
