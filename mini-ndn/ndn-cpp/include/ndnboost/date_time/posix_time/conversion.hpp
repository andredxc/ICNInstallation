...
#include <ndnboost/date_time/filetime_functions.hpp>
#include <ndnboost/date_time/c_time.hpp>
#include <ndnboost/date_time/time_resolution_traits.hpp> // absolute_value
#include <ndnboost/date_time/gregorian/conversion.hpp>
...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const ndnboost::posix_time::ptime& t) {
    std::tm timetm = ndnboost::gregorian::to_tm(t.date());
    ndnboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const ndnboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    ndnboost::gregorian::date d = ndnboost::gregorian::date_from_tm(timetm);
    return ptime(d, time_duration(timetm.tm_hour, timetm.tm_min, timetm.tm_sec));
  }


#if defined(NDNBOOST_HAS_FTIME)
  ...
  {
    return ndnboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // NDNBOOST_HAS_FTIME

} } //namespace ndnboost::posix_time
