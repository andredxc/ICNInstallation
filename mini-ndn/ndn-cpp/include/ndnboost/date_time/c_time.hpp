...
#endif // NDNBOOST_NO_STDC_NAMESPACE
...
namespace date_time {
  ...
  struct c_time {
    public:
#if defined(NDNBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS)
	...
	#endif
        if (!result)
          ndnboost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
	...
	#endif
        if (!result)
          ndnboost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
#else // NDNBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
...
#endif // NDNBOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
  };
}} // namespaces
