...
#ifndef NDNBOOST_UUID_0552D49838DD11DD90146B8956D89593
#define NDNBOOST_UUID_0552D49838DD11DD90146B8956D89593
...
#include <ndnboost/exception/get_error_info.hpp>
...
#include <ndnboost/exception/current_exception_cast.hpp>
namespace
ndnboost
    {
    namespace
    exception_detail
        {
        std::string diagnostic_information_impl( ndnboost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        ndnboost::exception const * be=current_exception_cast<ndnboost::exception const>();
...
ndnboost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( ndnboost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef NDNBOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<ndnboost::exception const *>(se);
            if( !se )
            ...
            if( be && verbose )
                ...
                if( !f && !l && !fn )
                    tmp << "Throw location unknown (consider using NDNBOOST_THROW_EXCEPTION)\n";
                else
                    {
                    if( f )
                        {
            ...
            if ( verbose )
                tmp << std::string("Dynamic exception type: ") <<
                    /*units::detail::demangle*/((be?(NDNBOOST_EXCEPTION_DYNAMIC_TYPEID(*be)):(NDNBOOST_EXCEPTION_DYNAMIC_TYPEID(*se))).type_->name()) << '\n';
#endif
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce ndnboost::diagnostic_information_what()";
#ifndef NDNBOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
