...
#ifndef NDNBOOST_INTRUSIVE_SLIST_HPP
#define NDNBOOST_INTRUSIVE_SLIST_HPP
...
#include <ndnboost/intrusive/slist_hook.hpp>
#include <ndnboost/intrusive/circular_slist_algorithms.hpp>
#include <ndnboost/intrusive/linear_slist_algorithms.hpp>
#include <ndnboost/intrusive/pointer_traits.hpp>
#include <ndnboost/intrusive/link_mode.hpp>
#include <ndnboost/intrusive/options.hpp>
...
class slist_impl
{
   ...
   static const bool has_container_from_iterator =
        ndnboost::is_same< header_holder_type, detail::default_header_holder< node_traits > >::value;
   ...
   //noncopyable
   NDNBOOST_MOVABLE_BUT_NOT_COPYABLE(slist_impl)

   static const bool safemode_or_autounlink = is_safe_autounlink<value_traits::link_mode>::value;

   //Constant-time size is incompatible with auto-unlink hooks!
   NDNBOOST_STATIC_ASSERT(!(constant_time_size && ((int)value_traits::link_mode == (int)auto_unlink)));
   //Linear singly linked lists are incompatible with auto-unlink hooks!
   NDNBOOST_STATIC_ASSERT(!(linear && ((int)value_traits::link_mode == (int)auto_unlink)));
   //A list with cached last node is incompatible with auto-unlink hooks!
   NDNBOOST_STATIC_ASSERT(!(cache_last && ((int)value_traits::link_mode == (int)auto_unlink)));
   ...
   static node_ptr get_last_node(detail::bool_<false>)
   {
      ...
      NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(cache_last);
      return node_ptr();
   }
   ...
   typedef typename ndnboost::intrusive::value_traits_pointers
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl(NDNBOOST_RV_REF(slist_impl) x)
      : data_(::ndnboost::move(x.priv_value_traits()))
   {
      this->priv_size_traits().set_size(size_type(0));
      node_algorithms::init_header(this->get_root_node());
      //nothrow, no need to rollback to release elements on exception
   ...
   //! <b>Effects</b>: to-do
   //!
   slist_impl& operator=(NDNBOOST_RV_REF(slist_impl) x)
   ...
   void push_front(reference value)
   {
      ...
      if(safemode_or_autounlink)
         NDNBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(to_insert));
      if(cache_last){
         if(this->empty()){
   ...
   {
      NDNBOOST_STATIC_ASSERT((cache_last));
      node_ptr n = priv_value_traits().to_node_ptr(value);
      if(safemode_or_autounlink)
         NDNBOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(n));
   ...
   {
      NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(++const_iterator(before_f), l) == difference_type(n));
      if(safemode_or_autounlink){
   ...
   {
      NDNBOOST_STATIC_ASSERT(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
      const_iterator it(prev);
      ++it;
      node_ptr to_erase(it.pointed_node());
      ++it;
   ...
   {
      NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(before_f, before_l) == difference_type(n));
      this->priv_splice_after
         (prev_pos.pointed_node(), x, before_f.pointed_node(), before_l.pointed_node());
      if(constant_time_size){
   ...
   void sort(Predicate p)
   {
      if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
         ...
         while(!this->empty()){
            ...
            NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(counter[i].empty());
   ...
   {
      NDNBOOST_STATIC_ASSERT((!stateful_value_traits));
   ...
   {
      NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(value)));
   ...
   {
      reference r =*pointer_traits<pointer>::const_cast_from(pointer_traits<const_pointer>::pointer_to(value));
      NDNBOOST_INTRUSIVE_INVARIANT_ASSERT (linear || !node_algorithms::inited(this->priv_value_traits().to_node_ptr(r)));
   ...
   {
      if(n){
         NDNBOOST_INTRUSIVE_INVARIANT_ASSERT(n > 0);
         NDNBOOST_INTRUSIVE_INVARIANT_ASSERT
   ...
   static slist_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      ...
      NDNBOOST_STATIC_ASSERT(!linear);
      NDNBOOST_STATIC_ASSERT((has_container_from_iterator));
...
#if defined(NDNBOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(NDNBOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class T, class ...Options>
#else
      ...
      >::type
{
   ...
   NDNBOOST_STATIC_ASSERT((detail::is_same<typename Base::value_traits::value_type, T>::value));
   NDNBOOST_MOVABLE_BUT_NOT_COPYABLE(slist)
   ...
   slist(NDNBOOST_RV_REF(slist) x)
      :  Base(::ndnboost::move(static_cast<Base&>(x)))
   {}

   slist& operator=(NDNBOOST_RV_REF(slist) x)
   {  return static_cast<slist &>(this->Base::operator=(::ndnboost::move(static_cast<Base&>(x))));  }
...
#endif //NDNBOOST_INTRUSIVE_SLIST_HPP
