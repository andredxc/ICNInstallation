...
class AnyMatchSwitch(app_manager.RyuApp):
	...
	Router = namedtuple('Router', 'dpid port mac int_ip nat_ip')
        self.routers = {0x01 : Router(0x01, 2, "00:00:00:00:00:01", "127.0.0.1", None)}
	...
	#ICN Multicast packet
	elif eth.ethertype == ether_types.ETH_TYPE_IP and ip.dst == "10.0.0.2" and ip.proto == inet.IPPROTO_UDP and dgram.dst_port == 6363:
		LOG.warn("AnyMatchSwitch: Accepted incoming packet from %s at switch %d, port %d, for reason %s"%(eth.src,dpid,in_port,reason))
		...
		exp_match = parser.OFPMatch(eth_type=0x0800, ip_proto=0x11, udp_dst=6363, exec_bpf=bpfOfpmatch)
