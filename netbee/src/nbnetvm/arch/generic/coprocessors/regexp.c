...
int re_err_offset;
	...
	{
		fprintf(stderr, "Error compiling regexp at character %d: %s\n", re_err_offset, re_err);
		rcp = NULL;
	}
	else
	{
		...
		if(w)
		{
			rcp->re_extra = pcre_study (re, PCRE_STUDY_JIT_COMPILE, &re_err);  //This setting need PCRE 8.20 or greater
		}else
		{
			if(!print_err_msg)
			{
				print_err_msg = 1;
				fprintf(stderr, "The PCRE version is older then 8.20 the JIT compile will be not used and matching may be slow\n");
			}
		...
		if (re_err != NULL)
		{
			fprintf(stderr, "Cannot study regular expression \"%s\":\n%s", pattern, re_err);
...
int32_t nvmRegExpCoproInjectData (nvmCoprocessorState *c, uint8_t *data) {
	...
	uint16_t g = 0, pattern_length = 0, flags_length = 0; // i = 0,
	int32_t out;
	char flags[20], *p;
	...
	for (g = 0; g < redata -> patterns_no; g++) {
		...
		if (nvmRegExpCoproNewPattern (&(redata -> patterns[g]), p, pattern_length, flags) == NULL)
			out= nvmFAILURE;
...
static uint32_t nvmRegExpCoproTestPattern (uint16_t pattern_id, uint8_t *haystack, uint32_t haylen,
	...
	if (pattern_id < redata -> patterns_no) {
		...
		m = pcre_exec (rcp -> re, rcp -> re_extra, (char*) haystack + start_offset, haylen, 0, 0, ovector, OVECTSIZE);   //It can give PCRE_ERROR_JIT_STACKLIMIT error if the 32K stack size is not enough
...
int32_t nvmCoproRegexpCreate(nvmCoprocessorState *regexp)
{
	...
	for (i=0; i<4; i++)
	{
		...
		regexp->ProfCounter[i]->TicksDelta= nbProfilerGetMeasureCost();
	}
	regexp->ProfCounter_tot=calloc (1, sizeof(nvmCounter));
	regexp->ProfCounter_tot->TicksDelta= nbProfilerGetMeasureCost();
#endif
