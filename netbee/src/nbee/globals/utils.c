...
char *Start;
	...
	// FULVIO: we should use the same code already tested for the sstrncpy here, as the current code does not check that the buffer is large enough
...
char *Start;
	...
	// Check that the target buffer was large enough
	if ((Destination == EndPos) && (*Destination != 0))
	{
		errorprintf(__FILE__, __FUNCTION__, __LINE__, "Warning: the target buffer is not large enough to keep data; data has been truncated.");
		*Destination= 0;
